"""
This script converts source code snippets into embeddings using GraphCodeBERT.
...
# 2. Select Model and Pooling
BASE_MODEL_NAME = "microsoft/graphcodebert-base"
model_choice = input("\nSelect base model (0) or fine-tuned model (1): ").strip()
...
if model_choice == "0":
    ...
    while True:
        p_choice = input("Select pooling for fine-tuned model: CLS (0) or Mean (1): ").strip()
        if p_choice in {"0", "1"}:
            pool_type = "cls" if p_choice == "0" else "mean"
            break
        print("Invalid input.")
    
    model_path = f"../fine_tuned_models/graphcodebert-clone-{language}-{pool_type}"
    suffix = f"tuned_{pool_type}"
...
TRAIN_SAVE_LOC = f"../data/embeddings/GraphCodeBERT/{prefix}_graphcodebert_embeddings_{suffix}.jsonl"
VAL_SAVE_LOC = f"../data/embeddings/GraphCodeBERT/{prefix}_graphcodebert_embeddings_{suffix}_val.jsonl"
...
def get_embedding(code_snippet):
    ...
    with torch.no_grad():
        ...
        if pool_type == "mean":
            ...
            embedding = (torch.sum(hidden * mask, dim=1) / torch.clamp(mask.sum(dim=1), min=1e-9)).squeeze(0)
        else:
            # CLS Pooling
            embedding = hidden[:, 0, :].squeeze(0)
...
def process_and_save(dataset, output_file, target_total):
    ...
    with open(output_file, 'w') as f:
        ...
        for i, example in enumerate(dataset):
            if count_0 >= target_per_label and count_1 >= target_per_label: break
...
if __name__ == "__main__":
    ...
    try:
        # Step 1: Training Set Generation (Only if not in API mode)
        if mode != "api":
            print("\n--- Starting Training Set Generation ---")
            process_and_save(ds, TRAIN_SAVE_LOC, DATASET_SIZE)
        else:
        ...
        process_and_save(ds_val, VAL_SAVE_LOC, VAL_SIZE)
